/*
 * Display Names support, ported from the Mobius fork, treated as a design
 * reference rather than a literal port (see the accompanying commit
 * messages). NameInfo mirrors Mobius's own class essentially exactly, since
 * it has no dependency on anything that has changed in current core.
 */
using System;
