...
| MoneyServer Compatibility | Original service, datastore and DTL/NSL connector remain separately deployable; selection guard prevents registration when unselected; configuration and CurrencyServer URL validation fail closed; one resident's failed MoneyServer login no longer changes module-wide availability for every region; simulator XML-RPC exposure is limited to the three MoneyServer callbacks required for paid-object delivery, balance refresh and user alerts, with teardown after the final region; prototype GetBalance/AddBanker/SendMoney/MoveMoney handlers are private and unregistered; simulated land-preparation endpoints that could succeed without a debit are not registered; public legacy request bodies are bounded; viewer currency purchases default off; removed regions are purged and unregister their money interface, and `Close()` independently tears down remaining region hooks | Focused compatibility connector and complete Release builds passed after callback-surface reduction; ContinuumEconomy does not load or redirect MoneyServer | Run the legacy compatibility matrix against a cloned MoneyServer database before enabling viewer purchases; verify required callbacks and negative probes for removed prototype methods, concurrent successful/failed logins on separate regions, region removal/reload, backend replacement, land settlement through the authenticated region `TransferMoney` path and oversized/chunked request rejection |
...
| Groups | OpenSim Dev Groups remains the service/module base, with reservation-safe creation and enrollment fees plus Gunthar-compatible custom invitation text. Both shared modules detach all region and resident callbacks, unregister their interfaces, and perform the same teardown after partial startup failure, explicit region removal or module close. Cross-region resident lookup and group-chat routing use locked scene snapshots; invitation/ejection/update paths tolerate the last region disappearing instead of indexing an empty list | Focused Groups and complete Release builds pass with zero errors | Test create/join fees, roles, notices, invitations, ejections, group chat, crossings, two regions in one process, concurrent removal/reload, service outage/recovery, Hypergrid groups and shutdown with connected residents |
| Offline IM | OpenSim Dev's local/Robust offline-message service remains the implementation base. The shared region connector owns exactly one undelivered-message subscription across hosted regions, unregisters callbacks during removal/close, tolerates a temporarily absent transfer module, snapshots region lookup safely, and contains service exceptions. Local storage serializes count/store so concurrent senders cannot exceed the configured per-recipient cap, performs old-message maintenance for every configured service rather than once process-wide, and removes unreadable poison rows without blocking every later login | Focused OfflineIM and complete Release builds pass with zero errors | Test direct, object, inventory-offer, group notice and invitation delivery across two regions; include concurrent sends at the exact cap, recipient login, corrupt-row recovery, duplicate retrieval, simulator/Robust restart, service outage/recovery and database-provider parity |
