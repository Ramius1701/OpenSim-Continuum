...
| MoneyServer Compatibility | Original service, datastore and DTL/NSL connector remain separately deployable; selection/configuration fail closed and one resident login failure cannot disable every region. Simulator XML-RPC exposure is limited to three required callbacks; prototype/unsafe simulated-land endpoints remain unregistered; request bodies are bounded and viewer purchases default off. Region hooks fully tear down. Service cleanup/stipends are single-flight, shutdown disposes timers before process termination, and startup creates only the one database connection manager actually used by the XML-RPC service instead of duplicating its configured pool | Focused compatibility service/connector and complete Release builds pass; ContinuumEconomy does not load or redirect MoneyServer | Run the legacy compatibility matrix against a cloned MoneyServer database; verify callbacks and negative probes, connection counts, concurrent logins, region reload, backend replacement, authenticated land settlement, oversized/chunked rejection, deliberately slow cleanup/stipend work, console shutdown and service restart |
