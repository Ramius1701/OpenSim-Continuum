...
| Offline IM | OpenSim Dev's local/Robust offline-message service remains the implementation base. The shared region connector now owns exactly one undelivered-message subscription across all hosted regions, unregisters per-region interfaces and client callbacks during removal/close, tolerates a temporarily absent transfer module, snapshots region lookup safely, and contains storage/retrieval service exceptions instead of propagating them through simulator event dispatch | Focused OfflineIM and complete Release builds pass with zero errors | Test direct, object, inventory-offer, group notice and invitation delivery across two regions; include recipient login, duplicate retrieval, simulator/Robust restart, service outage/recovery, message limits and database-provider parity |
