...
| ContinuumEconomy service | Separate service, authenticated region calls, sessions, transfers, currency purchase controls, object-sale holds and land preparation | SQLite and PostgreSQL XML-RPC wire tests passed; PostgreSQL covers all trusted transfer/credit operations, replay, final balances and invalid-secret rejection; both providers retained balances and replay safety across a full service stop/start | Outage, concurrency, viewer and multi-region testing on a cloned grid |
| ContinuumEconomy connector | Separately selected module, viewer events, OpenSim money hooks, delivery-safe paid object sales, ledger-free zero-price object delivery, ordered land settlement, exact-debit enforcement and local balance notifications; no inbound legacy money RPC exposure on simulator HTTP ports; per-avatar login failures cannot disable the shared simulator module; script-supplied payment UUIDs are preserved for replay safety; removed regions are purged from shared lookup state; connected residents recover service sessions after a service-only restart and outage balance reads are reported as failures rather than successful zeroes | Connector Release build passed; focused .NET 8 land regressions passed rejection of missing/partial debits, exact-debit transfer and zero-price transfer | Exercise every viewer payment/fee path, service-only restart recovery, same-UUID script retry, rejected-login isolation, crossings, dynamic region removal, remote-simulator recipients and negative public-port probes |
