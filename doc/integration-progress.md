...
| MoneyServer Compatibility | Original service, datastore and DTL/NSL connector remain separately deployable; selection/configuration fail closed and one resident login failure cannot disable every region. Simulator XML-RPC exposure is limited to three required callbacks; unsafe prototype endpoints remain unregistered; bodies are bounded and viewer purchases default off. Region hooks fully tear down. Cleanup/stipends are single-flight, shutdown disposes timers first, and startup creates one database pool. Pool exhaustion releases the allocator lock during short waits and fails after 30 seconds. Legacy currency and account orchestration no longer reserves unused outer connections. Transfers lock the pending ledger row and both balances, then debit, credit and complete the ledger in one database transaction; missing recipients, insufficient funds, overflow, concurrent replay or any write failure roll back without changing balances. Shared debit, credit and system-credit boundaries reject missing transaction/account identifiers and negative amounts | Focused compatibility service/connector and complete Release builds pass; ContinuumEconomy does not load or redirect MoneyServer | Run the legacy compatibility matrix against a cloned MoneyServer database; verify callbacks/negative probes, pool saturation and recovery, concurrent logins, concurrent debit/replay, negative amounts, missing-recipient and injected-failure balance preservation, region reload, backend replacement, authenticated land settlement, oversized/chunked rejection, slow cleanup/stipend work, console shutdown and service restart |
