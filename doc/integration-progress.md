...
| Experiences | Grid service, providers, complete viewer capability family, SL-compatible collection/profile LLSD, region/parcel policy and scripting integration are present; grid and standalone profiles select the required service connector | Complete Release build passed after capability discovery, empty-list typing and safe profile/search serialization fixes | Runtime certification only: verify all floater tabs, permissions, KVP, estate/parcel policy, restarts and Hypergrid isolation |
...
| RegionWeb | Optional per-region public website remains separate from grid-wide WhiteCore WebUI; Estate Admin requires HTTPS and Secure cookies, request bodies are bounded, root handler capture is prevented, and the donor's duplicate embedded wallet is disabled | Complete Release build succeeds with zero warnings and zero errors after the focused change | Test public pages/media escaping, authorized configuration backup/edit/reload, token expiry, oversized/chunked bodies, proxy HTTPS behavior, concurrent requests and Windows paths |
| Other recovered tooling | Windows first-run tooling remains experimental | Complete Release build succeeds | Finish the tooling boundary review and clean-VM handoff matrix |
