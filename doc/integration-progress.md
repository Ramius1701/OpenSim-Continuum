...
| ContinuumEconomy connector | Separately selected module, viewer events, OpenSim money hooks, delivery-safe paid object sales, reservation-safe uploads/groups/classifieds, ledger-free zero-price object delivery, ordered land settlement, exact-debit enforcement and local balance notifications; no inbound legacy money RPC exposure on simulator HTTP ports; per-avatar login failures cannot disable the shared simulator module; script-supplied payment UUIDs are preserved for replay safety; removed regions are purged from shared lookup state; connected residents recover service sessions after a service-only restart and outage balance reads are reported as failures rather than successful zeroes | Complete Release build passed; PostgreSQL fee-reservation wire workflow passed; focused .NET 8 land regressions passed rejection of missing/partial debits, exact-debit transfer and zero-price transfer | Exercise benefit/fee failure injection, every viewer payment path, service-only restart recovery, same-UUID script retry, rejected-login isolation, crossings, dynamic region removal, remote-simulator recipients and negative public-port probes |
