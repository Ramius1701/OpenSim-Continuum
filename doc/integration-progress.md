...
| Experiences | Grid service, providers, CAPS, region policy and scripting integration are present; grid and standalone example profiles now select and configure the required service connector so viewer CAPS/tabs can register | SQLite, MySQL/MariaDB and PostgreSQL paths compile; full solution build pending for this checkpoint | Verify viewer panels, permissions, KVP, estate/parcel policy, restarts and Hypergrid isolation |
