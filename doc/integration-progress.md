...
| MoneyServer Compatibility | Original service, datastore and DTL/NSL connector remain separately deployable; selection/configuration fail closed and one resident login failure cannot disable every region. Simulator XML-RPC exposure is limited to the three required callbacks; prototype and unsafe simulated-land endpoints remain unregistered; request bodies are bounded and viewer purchases default off. Region hooks fully tear down. Service transaction cleanup and stipend processing are single-flight, shutdown blocks new timer work, and both timers detach/dispose before base shutdown can terminate the process | Focused compatibility service/connector and complete Release builds pass; ContinuumEconomy does not load or redirect MoneyServer | Run the legacy compatibility matrix against a cloned MoneyServer database; verify callbacks and negative probes, concurrent logins, region reload, backend replacement, authenticated land settlement, oversized/chunked rejection, deliberately slow cleanup/stipend work, console shutdown and service restart |
