...
| Groups | OpenSim Dev Groups remains the service/module base, with reservation-safe creation and enrollment fees plus Gunthar-compatible custom invitation text. Both the core Groups and Groups Messaging shared modules now detach all region and resident callbacks, unregister their interfaces, and perform the same teardown after partial startup failure, explicit region removal or module close | Focused Groups and complete Release builds pass with zero errors | Test create/join fees, roles, notices, invitations, group chat, crossings, two regions in one process, removal/reload, service outage/recovery, Hypergrid groups and shutdown with connected residents |
