...
| MoneyServer Compatibility | Original service, datastore and DTL/NSL connector remain separately deployable; selection guard prevents registration when unselected; unused prototype direct-transfer, duplicate-initialization and false-success fallback-credit APIs were removed; viewer currency purchases default off | Complete Release build succeeds; ContinuumEconomy does not load or redirect MoneyServer | Run the legacy compatibility matrix against a cloned MoneyServer database before enabling viewer purchases |
