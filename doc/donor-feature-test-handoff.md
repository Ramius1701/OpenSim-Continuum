...
| RegionWeb | optional addon module | Explicitly enable on a test region; verify public/region routes, HTTPS-only Estate Admin, Secure cookies, authentication and estate authorization, CSRF, request-size limits, escaping, path traversal resistance, inventory privacy, configuration backup/reload and concurrent requests. | Disabled by default; its legacy embedded wallet stays disabled because RegionCurrency is separate. Test Windows paths, reverse-proxy TLS handling and HG-originated content. |
...
| RegionCurrency | optional addon module | Begin with purchases/transfers disabled; test HTTPS rejection, Secure cookies, token expiry/cooldown, balance display and portal routes against one selected compatible backend, including unavailable service and malformed input. | It is not a ledger and must not be enabled beside another portal/economy path. PayPal requires its separate acknowledgement and remains sandbox-only experimental work. |
